...
## 서론
우리는 재난 상황에서 모두에게 똑같이 울리는 문자 대신, “지금 내가 무엇을 해야 하는가”를 바로 실행 가능한 형태로 바꿔주는 개인화 계층을 만들었다. 이 개인화는 위로용 멘트나 모호한 조언이 아니라, 현재 위험도·이동 필요성·주변 환경을 해석해서 “지금 즉시 움직여야 하는가 / 아직 판단해도 되는가”를 명확하게 나눠준다. 기존 시스템은 이 구분 자체가 없었다.

우리는 이 구조를 온톨로지로 모델링했다. 공개 신뢰 데이터 중 우선적으로 접근 가능한 지진 데이터를 출발점으로 삼고, 사용자가 받는 안내를 ‘즉시 실행해야 하는 행동 블록’과 ‘다음 선택지를 이해시키는 맥락 블록’으로 분리했다. 이것은 단순 알림이 아니라, 재난 직후 수십 초부터 그 다음 몇 분까지의 의사결정 흐름 전체를 엔지니어링 가능한 형태로 만든 것이다. 이 접근은 지진을 넘어 수해, 산불, 나아가 사전 예방 알림까지 그대로 확장 가능한 모델이며, 우리는 이번에 그 프레임을 실제로 구현했다. 이건 언젠가는 누군가 만들어야 하는 계층이라고 믿었고, 우리는 이번에 그걸 처음부터 끝까지 직접 시도했다. 앞으로의 재난 대응 시스템에서 이 레이어는 선택이 아니라 기본이 될 것이라고 본다.
